...
public class CarEngine : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            Debug.LogError("Path is not set correctly.");
            return; // Keluar jika path tidak valid
        }
    ...
    private void GetPath()
    {
        ...
        // Periksa jumlah titik jalur yang ditemukan
        if (path_objs.Length < 2) // Pastikan ada setidaknya satu node
        {
            Debug.LogError("Path group must have at least one child.");
            return; // Keluar jika tidak ada jalur
        }
        ...
        // Pastikan kita tidak memiliki referensi null
        if (index == 0)
        {
            Debug.LogError("No valid path nodes found.");
            path = null; // Set ke null untuk menghindari kesalahan lebih lanjut
        }
    }
